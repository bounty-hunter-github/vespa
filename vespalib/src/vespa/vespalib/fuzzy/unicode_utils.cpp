...
std::u32string utf8_string_to_utf32(std::u8string_view u8str) {
    ...
    std::u32string u32ret;
    u32ret.reserve(u8str.size()); // Will over-allocate for all non-ASCII
    while (utf8_reader.hasMore()) {
        u32ret += static_cast<char32_t>(utf8_reader.getChar());
    }
    return u32ret;
}

std::u32string utf8_string_to_utf32(std::string_view str) {
    return utf8_string_to_utf32(std::u8string_view(reinterpret_cast<const char8_t*>(str.data()), str.size()));
}
